...
cat("The formula to find the standard error is: sqrt((p_male * (1 - p_male)) / num_males + (p_female * (1 - p_female)) / num_females)")
cat("We chose z* = 1.96 because it corresponds to the critical value for a 95% confidence interval from the standard normal distribution.\n")
cat("The manually calculated 95% confidence interval is: ", CI_lower, "to ", CI_upper, "\n")
```
#### Part D
```{r warning=FALSE, message=FALSE, echo=FALSE}
cat("If we were to repeat this experiment with many random samples from the population, then we would expect that the difference in proportions of males and females who fold their arms with the left arm on top will lie within the calculated confidence interval 95% of the time from ", CI_lower, "to ", CI_upper)
...
```{r warning=FALSE, message=FALSE, echo=FALSE}
cat("The standard error represents the spread of the difference in proportions of males and females who fold their arms with the left arm on top across different random samples from the population. It measures how much the observed difference in proportions might vary if we were to take repeated samples.")
